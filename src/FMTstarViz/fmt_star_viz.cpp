...
FMTstarViz::FMTstarViz( QWidget *parent ) :
    QLabel(parent) {
    mp_tree = NULL;
}

void FMTstarViz::setTree( FMTstar* p_tree ) {
    mp_tree = p_tree;
}


void FMTstarViz::paintEvent( QPaintEvent * e ) {
    QLabel::paintEvent(e);
    ...
    if(mp_tree) {
        ...
        for( std::list<TreeNode*>::iterator it= mp_tree->get_nodes().begin(); it!=mp_tree->get_nodes().end();it++ ) {
            TreeNode* p_node = (*it);
            if(p_node) {
                if(p_node->mp_parent) {
                ...
                /*
                for(std::list<TreeNode*>::iterator itc= p_node->m_child_nodes.begin(); itc!=p_node->m_child_nodes.end();itc++) {
...
bool FMTstarViz::drawPath(QString filename) {
...
void FMTstarViz::drawPathOnMap(QPixmap& map) {
