...
public:
    KDNode2D(value_type x, value_type y) : POS2D(x, y) { mpTreeNode = NULL; }
    KDNode2D(POS2D & pos) : POS2D(pos) { mpTreeNode = NULL; }
    void setTreeNode(TreeNode* pNode) { mpTreeNode = pNode; }
    TreeNode* getTreeNode() { return mpTreeNode; }
protected:
    TreeNode* mpTreeNode;
};
